...
export default function DeletedCard({ item, onRefresh }: DeletedCardProps) {
  ...
  return (
    <div className="relative flex flex-col justify-between rounded-lg overflow-hidden border border-slate-200 bg-white shadow-sm opacity-80">
      ...
      <div className="flex flex-col pl-5 pr-4 py-3.5 space-y-3">
        <div className="flex items-center gap-1.5">
          <span className="inline-flex items-center gap-1 h-[22px] px-2 rounded-md text-[11px] font-semibold bg-slate-100 text-slate-500 border border-slate-200 select-none">
        ...
        {item.image_url && (
          <img src={item.image_url} className="w-full h-32 object-contain rounded-md border border-slate-200 bg-slate-50 grayscale" />
        )}
