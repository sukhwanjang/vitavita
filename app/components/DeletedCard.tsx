...
export default function DeletedCard({ item, onRefresh }: DeletedCardProps) {
  ...
  return (
    <div className="relative flex flex-col justify-between rounded overflow-hidden border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm opacity-80">
      ...
      <div className="flex flex-col pl-5 pr-4 py-3.5 space-y-3">
        <div className="flex items-center gap-1.5">
          <span className="inline-flex items-center gap-1 h-[22px] px-2 rounded text-[11px] font-semibold bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 select-none">
            <IconTrash className="w-3 h-3" />
            삭제됨
          </span>
        </div>
        <div>
          <p className="text-base font-bold text-slate-700 dark:text-slate-300 truncate">{item.company}</p>
